...
import styles from "./NewDescribeYourRequestModal.module.css";
...
const NewDescribeYourRequestModal = ({
  nextStep,
  progressPercent,
  sellers,
  setShowConfirmModal,
  ...
  const { requestId, buyerStep } = useSelector((state) => state.buyer);
  ...
  return (
    ...
    >
      <div>
        {buyerStep === 10 ? (
          <div className={styles.innerSpinnerWrap}>
            <span className={styles.innerSpinner} />
          </div>
        ) : (
          <>
            {/* ── Submitted heading ── */}
            <div className={styles.submittedRow}>
              <h6 className={styles.submittedHeading}>
                <VerifyIcon className={styles.submittedIcon} />
            ...
            {/* ── Textarea ── */}
            <textarea
              className={`${styles.textarea} ${textError ? styles.textareaError : ""}`}
              value={text}
            ...
            {fileError && (
              <span className={styles.fieldError}>Please upload a file.</span>
            )}
...
export default NewDescribeYourRequestModal;
